import { Plus, Pencil, Clock } from 'lucide-react';
import { setRequestLocale, getTranslations } from 'next-intl/server';
import { Link } from '@/i18n/navigation';
import { Table, THead, TBody, TR, TH, TD } from '@/components/ui/table';
import { StatusPill } from '@/components/ui/status-pill';
import { EmptyState } from '@/components/ui/empty-state';
import { listShifts } from '@/lib/queries/shifts';
import { i18n } from '@/lib/validations/i18n';

const DAY_KEYS = [
  'Admin.shifts.day_short.sun',
  'Admin.shifts.day_short.mon',
  'Admin.shifts.day_short.tue',
  'Admin.shifts.day_short.wed',
  'Admin.shifts.day_short.thu',
  'Admin.shifts.day_short.fri',
  'Admin.shifts.day_short.sat',
] as const;

function trimSeconds(t: string): string {
  return t.length >= 5 ? t.slice(0, 5) : t;
}

export default async function AdminShiftsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);

  const rows = await listShifts();
  const t = await getTranslations('Admin.shifts');
  const tCommon = await getTranslations('Common');
  const tAll = await getTranslations();

  return (
    <div className="mx-auto max-w-7xl px-6 py-8">
      <div className="flex items-end justify-between border-b border-border pb-6">
        <div>
          <h1 className="text-2xl font-semibold">{t('title')}</h1>
          <p className="mt-1 text-sm text-fg-secondary">{t('description')}</p>
        </div>
        <Link
          href="/admin/shifts/new"
          className="inline-flex h-8 items-center gap-2 rounded-md bg-accent px-3 text-sm font-medium text-white hover:bg-accent-hover"
        >
          <Plus className="h-4 w-4" strokeWidth={1.75} aria-hidden />
          {t('new_cta')}
        </Link>
      </div>

      <div className="mt-6">
        {rows.length === 0 ? (
          <EmptyState icon={Clock} title={t('empty_title')} description={t('empty_description')} />
        ) : (
          <Table>
            <THead>
              <tr>
                <TH>{t('columns.campaign')}</TH>
                <TH>{t('columns.location')}</TH>
                <TH>{t('columns.time')}</TH>
                <TH>{t('columns.days')}</TH>
                <TH>{t('columns.status')}</TH>
                <TH numeric>
                  <span className="sr-only">{tCommon('edit')}</span>
                </TH>
              </tr>
            </THead>
            <TBody>
              {rows.map((row) => (
                <TR key={row.id}>
                  <TD>
                    <p className="font-medium">{i18n(row.campaign_name_i18n, locale)}</p>
                  </TD>
                  <TD>
                    <span className="text-fg-secondary">
                      {i18n(row.location_name_i18n, locale)}
                    </span>
                  </TD>
                  <TD>
                    <span dir="ltr" className="font-mono text-xs tabular-nums">
                      {trimSeconds(row.start_time)} – {trimSeconds(row.end_time)}
                    </span>
                  </TD>
                  <TD>
                    <span className="text-xs text-fg-secondary">
                      {row.days_of_week.length === 0
                        ? '—'
                        : row.days_of_week
                            .slice()
                            .sort((a, b) => a - b)
                            .map((d) => tAll(DAY_KEYS[d] ?? DAY_KEYS[0]))
                            .join(' · ')}
                    </span>
                  </TD>
                  <TD>
                    <StatusPill
                      variant={row.active ? 'success' : 'neutral'}
                      label={row.active ? tCommon('active') : tCommon('inactive')}
                    />
                  </TD>
                  <TD numeric>
                    <Link
                      href={`/admin/shifts/${row.id}/edit`}
                      className="inline-flex h-7 w-7 items-center justify-center rounded-md text-fg-secondary hover:bg-bg-hover hover:text-fg"
                      aria-label={tCommon('edit')}
                    >
                      <Pencil className="h-3.5 w-3.5" strokeWidth={1.75} aria-hidden />
                    </Link>
                  </TD>
                </TR>
              ))}
            </TBody>
          </Table>
        )}
      </div>
    </div>
  );
}
