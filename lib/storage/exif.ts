...
//
// Node-side mirror of supabase/functions/_shared/exif.ts. Used by the
// Phase 4 activity-photo upload Route Handler. Update BOTH files together
// when the EXIF logic changes — neither uses any runtime-specific API, so a
// verbatim byte-for-byte mirror is intentional.
//
...
// Indexed Uint8Array access in TS strict/noUncheckedIndexedAccess returns
// `number | undefined`. This helper narrows to `number` (0 when out of
// bounds), because the parser already checks length invariants at each step.
function b(buf: Uint8Array, off: number): number {
  return buf[off] ?? 0;
}
...
export function stripJpegMetadata(input: Uint8Array): Uint8Array {
  ...
  while (i < input.length) {
    ...
    if (b(input, i) !== 0xff) {
      throw new JpegParseError(`Expected marker at offset ${i}, got ${b(input, i)}`);
    }
    let marker = b(input, i + 1);
    ...
    if (markerCode === SOS) {
      ...
      for (let k = i; k < input.length; k++) out.push(b(input, k));
      break;
    }
    if (markerCode === EOI) {
      out.push(0xff, 0xd9);
    ...
    if (!isAppN && !isComment) {
      for (let k = i; k < segEnd; k++) out.push(b(input, k));
    }
...
function parseJpegExifMinimalUnsafe(input: Uint8Array): ExifMinimal {
  ...
  let i = 2;
  while (i < input.length) {
    if (b(input, i) !== 0xff) return {};
    const marker = b(input, i + 1);
    ...
    if (marker === 0xe1) {
      ...
      if (
        segEnd - segStart >= 6 &&
        b(input, segStart) === 0x45 && // 'E'
        b(input, segStart + 1) === 0x78 && // 'x'
        b(input, segStart + 2) === 0x69 && // 'i'
        b(input, segStart + 3) === 0x66 && // 'f'
        b(input, segStart + 4) === 0x00 &&
        b(input, segStart + 5) === 0x00
      ) {
...
function parseTiff(buf: Uint8Array, start: number, end: number): ExifMinimal {
  ...
  const byte0 = b(buf, start);
  const byte1 = b(buf, start + 1);
  let little: boolean;
...
function makeReader(buf: Uint8Array, little: boolean): Reader {
  if (little) {
    return {
      u16: (off) => b(buf, off) | (b(buf, off + 1) << 8),
      u32: (off) =>
        (b(buf, off) |
          (b(buf, off + 1) << 8) |
          (b(buf, off + 2) << 16) |
  ...
  return {
    u16: (off) => (b(buf, off) << 8) | b(buf, off + 1),
    u32: (off) =>
      ((b(buf, off) << 24) |
        (b(buf, off + 1) << 16) |
        (b(buf, off + 2) << 8) |
