/**
 * Stock ledger — pure functions.
 *
 * Single source of truth for the Phase-5 ledger math. Imported by:
 *   - Server Actions in `app/[locale]/{admin,supervisor,promoter}/stock/*`
 *     (validate before INSERT; the DB trigger is the defense-in-depth layer).
 *   - The `stock-reconcile` Supabase Edge Function (via a byte-for-byte
 *     mirror at `supabase/functions/_shared/ledger.ts`).
 *   - `lib/stock/ledger.test.ts`, which asserts the full Almarai scenario
 *     from spec pages 13–14 end-to-end (the Phase 5 exit criterion).
 *
